...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $columnNames = [];
    $columnValues = [];
    $columns = getNumberOfColumns($conn, "orders");

    foreach ($_POST as $key => $value) {
        if (!empty($value) && $key != 'oid') {
            $expectedType = getColumnType($conn, 'orders', $key);
    ...
    if (!empty($columnNames) && !empty($columnValues)) {
        $primaryKeyColumn = getPrimaryKeyColumn($conn, 'orders');
        $primaryKeyValue = $_POST[$primaryKeyColumn];

        if (primaryKeyExists($conn, 'orders', $primaryKeyColumn, $primaryKeyValue)) {
            ...
            $insertQuery = "INSERT INTO orders ($columnNamesString) VALUES ($columnValuesString)";
