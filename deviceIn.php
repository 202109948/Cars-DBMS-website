...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $columnNames = [];
    $columnValues = [];
    $columns = getNumberOfColumns($conn, "device");

    foreach ($_POST as $key => $value) {
        if (!empty($value) && $key != 'n') {
    ...
    if (!empty($columnNames) && !empty($columnValues)) {
        $primaryKeyColumn = getPrimaryKeyColumn($conn, 'device');
        $primaryKeyValue = $_POST[$primaryKeyColumn];

        if (primaryKeyExists($conn, 'device', $primaryKeyColumn, $primaryKeyValue)) {
            ...
            $insertQuery = "INSERT INTO device ($columnNamesString) VALUES ($columnValuesString)";
