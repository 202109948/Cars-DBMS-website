...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $columnNames = [];
    $columnValues = [];
    $columns = getNumberOfColumns($conn, "address");

    foreach ($_POST as $key => $value) {
        if (!empty($value) && $key != 'Aid') {
            $expectedType = getColumnType($conn, 'address', $key);
    ...
    if (!empty($columnNames) && !empty($columnValues)) {
        $primaryKeyColumn = getPrimaryKeyColumn($conn, 'address');
        $primaryKeyValue = $_POST[$primaryKeyColumn];

        if (primaryKeyExists($conn, 'address', $primaryKeyColumn, $primaryKeyValue)) {
            ...
            $insertQuery = "INSERT INTO address ($columnNamesString) VALUES ($columnValuesString)";
