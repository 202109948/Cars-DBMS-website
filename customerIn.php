...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $columnNames = [];
    $columnValues = [];
    $columns = getNumberOfColumns($conn, "customer");
    ...
    if (!empty($columnNames) && !empty($columnValues)) {
        $primaryKeyColumn = getPrimaryKeyColumn($conn, 'customer');
        $primaryKeyValue = $_POST[$primaryKeyColumn];

        if (primaryKeyExists($conn, 'customer', $primaryKeyColumn, $primaryKeyValue)) {
            ...
            $insertQuery = "INSERT INTO customer ($columnNamesString) VALUES ($columnValuesString)";
