...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $columnNames = [];
    $columnValues = [];
    $columns = getNumberOfColumns($conn, "manufacture");

    foreach ($_POST as $key => $value) {
        if (!empty($value) && $key != 'mname') {
    ...
    if (!empty($columnNames) && !empty($columnValues)) {
        $primaryKeyColumn = getPrimaryKeyColumn($conn, 'manufacture');
        $primaryKeyValue = $_POST[$primaryKeyColumn];

        if (primaryKeyExists($conn, 'manufacture', $primaryKeyColumn, $primaryKeyValue)) {
            ...
            $insertQuery = "INSERT INTO manufacture ($columnNamesString) VALUES ($columnValuesString)";
